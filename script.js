...
function openGame(game) {
    ...
    document.getElementById('game2048Screen').style.display = 'none';
    document.getElementById('snakeScreen').style.display = 'none';
    
    const screen = document.getElementById(game + 'Screen');
    if (screen) {
        screen.style.display = 'block';
        if (game === 'quiz') getAIQuestion();
        if (game === 'game2048') init2048();
...
async function getAIQuestion() {
    ...
    try {
        const resp = await fetch('https://text.pollinations.ai/openai', {
            ...
            body: JSON.stringify({
                model: 'openai',
                messages: [{ role: 'user', content: `Придумай вопрос для викторины. Тема: "${topic}". Сложность: "${difficulty}". Ответ — строго JSON: {"question":"...","options":["a","b","c","d"],"correctIndex":0}. correctIndex — 0,1,2 или 3.` }],
                temperature: 0.9
            })
        });
        ...
        document.getElementById('optionsGrid').innerHTML = q.options.map((o, i) =>
            `<button class="option-btn" onclick="checkQuiz(${i}, this)">${o}</button>`
        ).join('');
    } catch (e) {
        document.getElementById('questionText').textContent = '😵 Ошибка генерации. Попробуй ещё раз.';
    }
